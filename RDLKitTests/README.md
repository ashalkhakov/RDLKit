...
Checks for **RDLKit**, the report library. The app has its own target,
`../RDLDesignerTests`.

An **XCTest** suite for the generator — plain `XCTestCase` and `XCTAssertTrue`,
on macOS and on GNUstep.

The checks themselves (`RDLChecks`) are plain Foundation functions returning
...
Or open `../RDLKit.xcodeproj`, scheme **RDLKitTests**, Product → Test.
...
cd ../RDLKit && make
cd ../RDLKitTests && make run-tests
```

`make` builds `RDLKitTests.bundle`; `make run-tests` runs it through `xctest`.
