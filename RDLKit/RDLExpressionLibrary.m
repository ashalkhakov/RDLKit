...
//
// The runtime library: every function the expression language offers, and the
// table the evaluator finds them in. Reading and evaluating an expression are
...
static id RDLVisualBasicNumberText(id value, RDLVisualBasicNumberStyle style, NSArray *options, NSString *language) {
  if (RDLIsError(value))
    return value;
  if (RDLIsNothing(value))
    return @"";
...
// -- a form that chooses which to work out is the compiler's business, not a
// function's -- and `scope` what the report knows.
//
...
static id RDLCall(NSString *name, NSArray *vals, RDLEvalScope *scope) {
