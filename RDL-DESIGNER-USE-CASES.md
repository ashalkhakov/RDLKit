...
| DAT-09 | Give values and render | Centre > Dataset tab with no dataset selected (the render inputs): type values, then preview | The value reaches the query and the render. *Guarded by `testAParameterGivenInTheDesignerReachesTheQuery`, `testAParameterAppliesToWhatTheGeneratorRenders`, `testSeveralValuesAreGivenInTheDataPane`* |
