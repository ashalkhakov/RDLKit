...
| WIN-04 | Zoom | ⌘= and ⌘- , and the zoom control | 40% to 400%, the keyboard stepping through exactly what the control lists; the rulers, grid, handles and tablix band all scale with it. *Guarded by `testTheCanvasZoomsToFourHundredPercent`, `testZoomingStepsThroughWhatTheControlLists`, `testPreviewZoomAndRulers`, `testTheTablixHandleBandScalesWithTheZoom`* |
...
| CAN-04 | Nudge | Arrow keys, then hold | Moves by the grid step as the key repeats; a burst of keys is one undo step, and the panes are told once, when it is let go. *Guarded by `testABurstOfNudgesTellsThePanesOnce`* |
...
| INSP-02 | Common properties | Name, position, size, visibility, tooltip, page break, keep together | Each writes through and undoes; a name that is not an RDL name, or is another item's, is refused with a reason under the field. *Guarded by `testTheInspectorEditsWhatEveryItemHas`, `testRenamingSaysWhyItRefusesAndUndoes`* |
...
| TBL-13 | Delete a group | Pane menu > Delete Group; then Delete Group and Rows; then the buttons under the tree | The first keeps the rows, the second takes them; each button is offered only when it would work. *Guarded by `testDeletingAGroupKeepsItsRowsOrTakesThem`, `testTheGroupsPanesButtonsSayWhatTheyCanDo`* |
...
| # | Case | Steps | Expected |
|---|---|---|---|
| DAT-01 | Add a data source | Datasets tab > data sources > + | Kind (JSON, XML, delimited text) and a document to read — content kept in the report or a file, and a source may hold both and say which it reads; an unknown provider is shown read-only. *Guarded by `testDataSourcesAndDatasetsAreEditedApart`, `testAnUnknownDataProviderIsShownReadOnly`* |
...
| DAT-09 | Give values and render | The preview's Parameters… panel, or Centre > Dataset tab with no dataset selected: type values, then OK. Use the report's defaults puts every value back to what the report works out; each data source says whether it reads the data kept in the report or a file | The value reaches the query and the render. *Guarded by `testThePreviewAsksForTheParametersBeforeItRenders`, `testCancellingTheInputsPanelChangesNothing`, `testTheInputsPanelChoosesBetweenTheKeptDataAndAFile`, `testTheInputsPanelGoesBackToTheReportsDefaults`, `testAParameterGivenInTheDesignerReachesTheQuery`, `testAParameterAppliesToWhatTheGeneratorRenders`, `testSeveralValuesAreGivenInTheDataPane`* |
