...
static NSString *const kPicaImportDataSetName = @"Data";
...
static const CGFloat kPicaPointsPerInch = 72.0;

static inline CGFloat PicaPointsToInches(CGFloat points) {
  return points / kPicaPointsPerInch;
}

static inline CGFloat PicaInchesToPoints(CGFloat inches) {
  return inches * kPicaPointsPerInch;
}
...
static const CGFloat kPicaMinimumTextWidthPoints = 8.0;
...
// scaffold needs.
static const CGFloat kPicaEmptyParagraphHeight = 0.14;
...
static const CGFloat kPicaMinimumBandHeight = 0.25;
...
static const CGFloat kPicaNominalRowHeight = 0.25;
...
static NSAttributedString *PicaAttributedRuns(NSArray<RDLImportRun *> *runs, RDLStyle *base,
...
static CGFloat PicaPoints(RDLLength *length, CGFloat fallback) {
...
static CGFloat PicaBoxHeight(NSAttributedString *text, CGFloat width, RDLStyle *style) {
  if ([text length] == 0 || width <= 0)
    return 0;
  CGFloat padX = PicaPoints(style.paddingLeft, 0) + PicaPoints(style.paddingRight, 0);
  CGFloat padY = PicaPoints(style.paddingTop, 0) + PicaPoints(style.paddingBottom, 0);
  CGFloat usable = MAX(PicaInchesToPoints(width) - padX, kPicaMinimumTextWidthPoints);
  ...
  return PicaPointsToInches(ceil(NSHeight(box)) + padY);
}
...
// box into a region.
static NSString *PicaRunExpression(RDLImportRun *run) {
  if (run.fieldName)
    return [NSString stringWithFormat:@"=First(Fields!%@.Value, \"%@\")", run.fieldName,
                                      kPicaImportDataSetName];
  return run.text ?: @"";
}
...
static NSString *PicaRunDisplayText(RDLImportRun *run) {
  if (run.fieldName)
...
// from that one".
static void PicaFillTextbox(RDLTextbox *box, NSArray<RDLImportRun *> *runs, RDLTextAlign alignment) {
  box.canGrow = NO;
  if ([runs count] == 0) {
    box.value = @"";
    return;
  ...
  if (uniform) {
    ...
    box.value = PicaRunExpression(only);
    if (only.style)
  ...
  for (RDLImportRun *run in runs) {
    ...
    out.value = PicaRunExpression(run);
    out.style = run.style;
    [para.runs addObject:out];
    [flattened appendString:PicaRunDisplayText(run)];
  }
...
static NSString *PicaFieldNameFromHeading(NSString *heading) {
...
static NSString *PicaCellText(RDLImportCell *cell) {
  NSMutableString *text = [NSMutableString string];
  for (RDLImportRun *run in cell.runs)
    [text appendString:PicaRunDisplayText(run)];
...
static NSArray<NSString *> *PicaFieldNamesForHeader(RDLImportRow *header, NSUInteger columnCount) {
  ...
  while ([names count] < columnCount) {
    ...
    if (cellIndex < [header.cells count]) {
      ...
      heading = PicaFieldNameFromHeading(PicaCellText(cell));
...
// where.
@interface PicaPlacement : NSObject
@property (nonatomic, strong) PicaNamer *namer;
...
static RDLTablix *PicaTablixForTable(RDLImportBlock *block, PicaNamer *namer, RDLStyle *base,
  ...
  for (RDLImportRow *importRow in block.rows) {
    ...
    for (RDLImportCell *importCell in importRow.cells) {
      ...
      PicaFillTextbox(box, importCell.runs, RDLTextAlignUnspecified);
      cell.item = box;
      ...
      rowHeight = MAX(rowHeight, PicaBoxHeight(PicaAttributedRuns(importCell.runs, base,
                                                                  RDLTextAlignUnspecified),
                                               cellWidth, box.style));
    }
    row.height = MAX(rowHeight, kPicaMinimumRowHeight);
...
static void PicaGiveTableADataSet(RDLTablix *tablix, RDLImportBlock *block,
                                  PicaPlacement *placement, CGFloat *outHeight) {
  ...
  NSArray<NSString *> *names = PicaFieldNamesForHeader(headerRow, columnCount);
...
@interface PicaColumnBox : NSObject
@property (nonatomic, assign) CGFloat x, width;
@end
@implementation PicaColumnBox
@end

static NSArray<PicaColumnBox *> *PicaColumnsOf(RDLImportSection *section, CGFloat bodyWidth) {
  ...
  for (NSInteger i = 0; i < count; i++) {
    PicaColumnBox *box = [[PicaColumnBox alloc] init];
...
@interface PicaSegment : NSObject
...
static RDLImportTabStop *PicaNextTabStop(RDLImportBlock *block, CGFloat interval, CGFloat pen,
                                         CGFloat limit) {
...
static NSArray<PicaSegment *> *PicaSegmentsOfBlock(RDLImportBlock *block, CGFloat interval,
                                                   CGFloat width, RDLStyle *base) {
  NSMutableArray<PicaSegment *> *segments = [NSMutableArray array];
  ...
  for (RDLImportRun *run in block.runs) {
    if (run.isTab) {
      if ([current count]) {
        [segments addObject:({
          PicaSegment *seg = [[PicaSegment alloc] init];
          seg.runs = current;
          seg;
        })];
  ...
  for (PicaSegment *segment in segments) {
    BOOL anything = NO;
  ...
  for (NSUInteger i = 0; i < [segments count]; i++) {
    PicaSegment *segment = segments[i];
    NSInteger tabs = [tabsBefore[i] integerValue];
    for (NSInteger t = 0; t < tabs; t++) {
      RDLImportTabStop *stop = PicaNextTabStop(block, interval, pen, width);
      if (stop == nil)
        break;
      pen = stop.position;
    ...
    segment.left = pen;
    NSAttributedString *text = PicaAttributedRuns(segment.runs, base, block.alignment);
    NSRect measured = [text boundingRectWithSize:NSMakeSize(CGFLOAT_MAX, CGFLOAT_MAX)
                                          options:NSStringDrawingUsesLineFragmentOrigin |
                                                  NSStringDrawingUsesFontLeading];
    pen += PicaPointsToInches(NSWidth(measured));
  }
  return segments;
}

static CGFloat PicaPlaceBlocks(NSArray<RDLImportBlock *> *blocks, NSMutableArray<RDLItem *> *into,
                               CGFloat x, CGFloat y, CGFloat width, PicaPlacement *placement) {
  PicaNamer *namer = placement.namer;
  ...
  for (RDLImportBlock *block in blocks) {
    if (block.kind == RDLImportBlockTable) {
      CGFloat height = 0;
      RDLTablix *tablix = PicaTablixForTable(block, namer, base, width, &height);
      // Every table gets a dataset; one with rows in it also gets columns the
      // designer can edit.
      PicaGiveTableADataSet(tablix, block, placement, &height);
      tablix.left = x;
      tablix.top = cursor;
    ...
    if (block.kind == RDLImportBlockImage) {
      ...
      cursor += image.height + PicaPointsToInches(block.spaceAfter);
      continue;
    }
    if (block.kind == RDLImportBlockRule) {
      ...
      [into addObject:line];
      cursor += MAX(block.imageHeight, kPicaRuleHeight);
      continue;
    }

    cursor += PicaPointsToInches(block.spaceBefore);
    NSAttributedString *text = PicaAttributedRuns(block.runs, base, block.alignment);
    if ([text length] == 0) {
      // An empty paragraph is vertical space, and the document meant it.
      cursor += MAX(PicaPointsToInches(block.spaceAfter), kPicaEmptyParagraphHeight);
      continue;
    }
    NSArray<PicaSegment *> *segments =
        PicaSegmentsOfBlock(block, placement.tabInterval, width, base);
    ...
    for (NSUInteger i = 0; i < [segments count]; i++) {
      ...
      CGFloat segmentWidth = MAX(right - segment.left, kPicaMinimumSegmentWidth);
      RDLTextbox *box = [[RDLTextbox alloc] init];
      box.name = [namer nameFor:kind];
      PicaFillTextbox(box, segment.runs, block.alignment);
      ...
      NSAttributedString *segmentText = PicaAttributedRuns(segment.runs, base, block.alignment);
      box.height = MAX(PicaBoxHeight(segmentText, box.width, box.style), kPicaMinimumBoxHeight);
    ...
    cursor += tallest + PicaPointsToInches(block.spaceAfter);
  }
  return cursor - y;
}
...
+ (RDLReport *)reportFromDocxData:(NSData *)data
  ...
  PicaNamer *namer = [[PicaNamer alloc] init];
  ...
  PicaPlacement *placement = [[PicaPlacement alloc] init];
  ...
  if ([document.fieldNames count]) {
    RDLDataSet *dataSet = [[RDLDataSet alloc] init];
    dataSet.name = kPicaImportDataSetName;
  ...
  if ([document.headerBlocks count]) {
    ...
    CGFloat height = PicaPlaceBlocks(document.headerBlocks, items, 0, 0, bodyWidth, placement);
    [report.pageHeader.items addObjectsFromArray:items];
    report.pageHeader.height = MAX(height, kPicaMinimumBandHeight);
  } else {
    report.pageHeader.height = 0;
  }
  if ([document.footerBlocks count]) {
    NSMutableArray *items = [NSMutableArray array];
    CGFloat height = PicaPlaceBlocks(document.footerBlocks, items, 0, 0, bodyWidth, placement);
    [report.pageFooter.items addObjectsFromArray:items];
    report.pageFooter.height = MAX(height, kPicaMinimumBandHeight);
  } else {
  ...
  while (index < [document.blocks count]) {
    ...
    RDLImportSection *section = sectionIndex < (NSInteger)[document.sections count]
                                    ...
                                    : first;
    NSArray<PicaColumnBox *> *columns = PicaColumnsOf(section, bodyWidth);
    if ([columns count] <= 1) {
      cursor += PicaPlaceBlocks(run, items, 0, cursor, bodyWidth, placement);
      continue;
    }
    ...
    for (RDLImportBlock *block in run) {
      CGFloat height = 0;
      if (block.kind == RDLImportBlockTable) {
        height = kPicaNominalRowHeight * (CGFloat)[block.rows count];
      } else {
        height = PicaBoxHeight(PicaAttributedRuns(block.runs, base, block.alignment),
                               [columns firstObject].width, [RDLStyle defaultStyle]) +
                 PicaPointsToInches(block.spaceBefore + block.spaceAfter);
      }
    ...
    for (NSUInteger c = 0; c < [columns count] && blockIndex < [run count]; c++) {
      ...
      CGFloat height =
          PicaPlaceBlocks(slice, items, columns[c].x, cursor, columns[c].width, placement);
  ...
  report.body.height = MAX(cursor, kPicaMinimumBodyHeight);
