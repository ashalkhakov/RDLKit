...
static const uint32_t kPicaEndOfCentralDirectorySig = 0x06054b50;
static const uint32_t kPicaCentralFileHeaderSig = 0x02014b50;
static const uint32_t kPicaLocalFileHeaderSig = 0x04034b50;

static const NSUInteger kPicaEOCDMinimumLength = 22;
// A ZIP comment can be 64 KB, and the end record sits before it.
static const NSUInteger kPicaMaxCommentLength = 0xFFFF;

@interface PicaZipEntry : NSObject
...
@end
@implementation PicaZipEntry
@end

@implementation RDLZipArchive {
  NSData *_data;
  NSMutableArray<NSString *> *_names;
  NSMutableDictionary<NSString *, PicaZipEntry *> *_entries;
}
...
static BOOL PicaReadU16(NSData *data, NSUInteger at, uint16_t *out) {
  if (at + 2 > [data length])
    return NO;
...
static BOOL PicaReadU32(NSData *data, NSUInteger at, uint32_t *out) {
  if (at + 4 > [data length])
    return NO;
  const uint8_t *b = (const uint8_t *)[data bytes] + at;
...
static NSError *PicaZipError(NSString *why) {
  return [NSError errorWithDomain:@"PicaKit"
                             code:10
...
- (BOOL)findEndOfCentralDirectoryIn:(NSData *)data at:(NSUInteger *)outAt {
  ...
  if (length < kPicaEOCDMinimumLength)
    return NO;
  NSUInteger earliest =
      length > kPicaEOCDMinimumLength + kPicaMaxCommentLength
          ? length - kPicaEOCDMinimumLength - kPicaMaxCommentLength
          : 0;
  for (NSUInteger at = length - kPicaEOCDMinimumLength + 1; at-- > earliest;) {
    uint32_t sig = 0;
    if (PicaReadU32(data, at, &sig) && sig == kPicaEndOfCentralDirectorySig) {
      *outAt = at;
      return YES;
    }
  }
...
- (BOOL)openData:(NSData *)data error:(NSError **)error {
  if ([data length] == 0) {
    if (error)
      *error = PicaZipError(@"the file is empty");
    return NO;
  }
  NSUInteger eocd = 0;
  if (![self findEndOfCentralDirectoryIn:data at:&eocd]) {
    if (error)
      *error = PicaZipError(@"not a ZIP archive: no end-of-central-directory record");
    return NO;
  }

  uint16_t entryCount = 0;
  uint32_t directoryOffset = 0;
  if (!PicaReadU16(data, eocd + 10, &entryCount) ||
      !PicaReadU32(data, eocd + 16, &directoryOffset)) {
    if (error)
      *error = PicaZipError(@"the end-of-central-directory record is truncated");
    return NO;
  }
  ...
  if (entryCount == 0xFFFF || directoryOffset == 0xFFFFFFFF) {
    if (error)
      *error = PicaZipError(@"Zip64 archives are not supported");
    return NO;
  }
  ...
  for (uint16_t i = 0; i < entryCount; i++) {
    uint32_t sig = 0;
    if (!PicaReadU32(data, at, &sig) || sig != kPicaCentralFileHeaderSig) {
      if (error)
        *error = PicaZipError(@"the central directory is corrupt");
      return NO;
    }
    uint16_t method = 0, nameLength = 0, extraLength = 0, commentLength = 0;
    uint32_t compressed = 0, uncompressed = 0, localOffset = 0;
    if (!PicaReadU16(data, at + 10, &method) || !PicaReadU32(data, at + 20, &compressed) ||
        !PicaReadU32(data, at + 24, &uncompressed) || !PicaReadU16(data, at + 28, &nameLength) ||
        !PicaReadU16(data, at + 30, &extraLength) || !PicaReadU16(data, at + 32, &commentLength) ||
        !PicaReadU32(data, at + 42, &localOffset)) {
      if (error)
        *error = PicaZipError(@"a central directory entry is truncated");
      return NO;
    }
    if (at + 46 + nameLength > [data length]) {
      if (error)
        *error = PicaZipError(@"an entry name runs past the end of the file");
      return NO;
    }
    ...
    if ([name length]) {
      PicaZipEntry *entry = [[PicaZipEntry alloc] init];
      entry.name = name;
      entry.method = method;
...
- (NSData *)compressedBytesFor:(PicaZipEntry *)entry {
  uint32_t sig = 0;
  if (!PicaReadU32(_data, entry.localHeaderOffset, &sig) || sig != kPicaLocalFileHeaderSig)
    return nil;
  uint16_t nameLength = 0, extraLength = 0;
  if (!PicaReadU16(_data, entry.localHeaderOffset + 26, &nameLength) ||
      !PicaReadU16(_data, entry.localHeaderOffset + 28, &extraLength))
    return nil;
...
static NSData *PicaInflate(NSData *input, uint32_t expectedSize) {
  if ([input length] == 0)
    return [NSData data];
  z_stream stream;
...
- (NSData *)dataForEntryNamed:(NSString *)name {
  PicaZipEntry *entry = _entries[name];
  if (entry == nil)
    return nil;
  ...
  if (entry.method != 8) // anything but deflate
    return nil;
  return PicaInflate(raw, entry.uncompressedSize);
}
