...
static NSString *PicaLocal(NSXMLNode *n) {
...
static NSArray<NSXMLElement *> *PicaKids(NSXMLElement *el, NSString *name) {
  ...
  for (NSXMLNode *n in [el children])
    if (n.kind == NSXMLElementKind && [PicaLocal(n) isEqualToString:name])
      [out addObject:(NSXMLElement *)n];
  return out;
}

static NSXMLElement *PicaKid(NSXMLElement *el, NSString *name) {
  return [PicaKids(el, name) firstObject];
}

// w:val, whatever prefix the document uses for the w namespace.
static NSString *PicaAttr(NSXMLElement *el, NSString *name) {
  if (el == nil)
    return nil;
  for (NSXMLNode *a in [el attributes])
    if ([PicaLocal(a) isEqualToString:name])
...
static NSString *PicaVal(NSXMLElement *el) {
  return PicaAttr(el, @"val");
}

static CGFloat PicaTwipsToInches(NSString *twips) {
...
static BOOL PicaToggleState(NSXMLElement *parent, NSString *name, BOOL *outOn) {
  NSXMLElement *el = PicaKid(parent, name);
  if (el == nil)
    return NO;
  NSString *v = PicaAttr(el, @"val");
  *outOn = !(v != nil && ([v isEqualToString:@"0"] || [v isEqualToString:@"false"] ||
                          [v isEqualToString:@"off"]));
  return YES;
}

static CGFloat PicaTwipsToPoints(NSString *twips) {
  return twips ? (CGFloat)([twips doubleValue] / 20.0) : 0;
}

#pragma mark - The stylesheet

static RDLTextAlign PicaAlignment(NSString *jc);

static RDLImportTabAlignment PicaTabAlignmentFromString(NSString *val) {
...
// be converted.
@interface PicaDocxContext : NSObject
@property (nonatomic, strong) RDLZipArchive *zip;
@property (nonatomic, strong) PicaStyleSheet *sheet;
...
@interface PicaStyleSheet : NSObject
...
@end
@implementation PicaDocxContext
@end

@implementation PicaStyleSheet
@end
...
static PicaStyleSheet *PicaReadStyleSheet(NSXMLElement *root) {
  if (root == nil)
    return nil;
  PicaStyleSheet *sheet = [[PicaStyleSheet alloc] init];
  NSMutableDictionary *byId = [NSMutableDictionary dictionary];
  for (NSXMLElement *style in PicaKids(root, @"style")) {
    NSString *styleId = PicaAttr(style, @"styleId");
    if ([styleId length] == 0)
      continue;
    byId[styleId] = style;
    ...
    if (sheet.defaultParagraphStyleId == nil &&
        [PicaAttr(style, @"type") isEqualToString:@"paragraph"] &&
        [PicaAttr(style, @"default") isEqualToString:@"1"])
      sheet.defaultParagraphStyleId = styleId;
  }
  sheet.styles = byId;
  NSXMLElement *docDefaults = PicaKid(root, @"docDefaults");
  sheet.defaultRunProperties = PicaKid(PicaKid(docDefaults, @"rPrDefault"), @"rPr");
  sheet.defaultParagraphProperties = PicaKid(PicaKid(docDefaults, @"pPrDefault"), @"pPr");
  return sheet;
}
...
static NSArray<NSXMLElement *> *PicaStyleChain(PicaStyleSheet *sheet, NSString *styleId,
  ...
  while ([current length] && ![seen containsObject:current]) {
    ...
    if (style == nil)
      break;
    NSXMLElement *properties = PicaKid(style, propertiesName);
    if (properties)
      [chain insertObject:properties atIndex:0];
    current = PicaVal(PicaKid(style, @"basedOn"));
  }
  return chain;
}
...
static void PicaApplyRunProperties(RDLStyle *into, NSXMLElement *rPr) {
  if (rPr == nil)
    return;
  NSXMLElement *fonts = PicaKid(rPr, @"rFonts");
  NSString *face = PicaAttr(fonts, @"ascii") ?: PicaAttr(fonts, @"hAnsi");
  if ([face length])
    into.fontFamily = face;
  NSString *size = PicaVal(PicaKid(rPr, @"sz"));
  if ([size length])
    into.fontSize = [RDLLength points:[size doubleValue] / 2.0]; // half-points
  BOOL on = NO;
  if (PicaToggleState(rPr, @"b", &on))
    into.fontWeight = on ? RDLFontWeightBold : RDLFontWeightNormal;
  if (PicaToggleState(rPr, @"i", &on))
  ...
  NSXMLElement *underline = PicaKid(rPr, @"u");
  if (underline)
    into.textDecoration = [PicaAttr(underline, @"val") isEqualToString:@"none"]
                              ? RDLTextDecorationNone
                              : RDLTextDecorationUnderline;
  if (PicaToggleState(rPr, @"strike", &on))
    into.textDecoration = on ? RDLTextDecorationLineThrough : RDLTextDecorationNone;
  NSString *color = PicaVal(PicaKid(rPr, @"color"));
...
static BOOL PicaStyleIsEmpty(RDLStyle *style) {
...
static RDLStyle *PicaRunStyle(NSXMLElement *rPr, PicaStyleSheet *sheet, NSString *paragraphStyleId) {
  if (sheet == nil && rPr == nil)
    return nil;
  RDLStyle *style = [[RDLStyle alloc] init];
  if (sheet) {
    PicaApplyRunProperties(style, sheet.defaultRunProperties);
    NSString *paragraphStyle = [paragraphStyleId length] ? paragraphStyleId
                                                         : sheet.defaultParagraphStyleId;
    for (NSXMLElement *properties in PicaStyleChain(sheet, paragraphStyle, @"rPr"))
      PicaApplyRunProperties(style, properties);
    for (NSXMLElement *properties in
         PicaStyleChain(sheet, PicaVal(PicaKid(rPr, @"rStyle")), @"rPr"))
      PicaApplyRunProperties(style, properties);
  }
  PicaApplyRunProperties(style, rPr);
  return PicaStyleIsEmpty(style) ? nil : style;
}
...
static void PicaApplyParagraphProperties(RDLImportBlock *block, NSXMLElement *pPr) {
  if (pPr == nil)
    return;
  NSXMLElement *jc = PicaKid(pPr, @"jc");
  if (jc)
    block.alignment = PicaAlignment(PicaAttr(jc, @"val"));
  NSXMLElement *spacing = PicaKid(pPr, @"spacing");
  if (PicaAttr(spacing, @"before"))
    block.spaceBefore = PicaTwipsToPoints(PicaAttr(spacing, @"before"));
  if (PicaAttr(spacing, @"after"))
    block.spaceAfter = PicaTwipsToPoints(PicaAttr(spacing, @"after"));
  NSString *outline = PicaVal(PicaKid(pPr, @"outlineLvl"));
  if (outline)
    block.outlineLevel = [outline integerValue];
  NSXMLElement *indent = PicaKid(pPr, @"ind");
  if (PicaAttr(indent, @"left"))
    block.indentLeft = PicaTwipsToInches(PicaAttr(indent, @"left"));
  NSXMLElement *tabs = PicaKid(pPr, @"tabs");
  if (tabs) {
    ...
    for (NSXMLElement *tab in PicaKids(tabs, @"tab")) {
      RDLImportTabStop *stop = [[RDLImportTabStop alloc] init];
      stop.position = PicaTwipsToInches(PicaAttr(tab, @"pos"));
      stop.alignment = PicaTabAlignmentFromString(PicaAttr(tab, @"val"));
...
static NSString *PicaMergeFieldName(NSString *instruction) {
  if (instruction == nil)
    return nil;
...
static NSArray<RDLImportRun *> *PicaCoalesce(NSArray<RDLImportRun *> *runs) {
  NSMutableArray *out = [NSMutableArray array];
...
static NSArray<NSValue *> *PicaPlaceholderRanges(NSString *text, NSArray<NSString *> **outNames) {
...
// starts in.
static NSArray<RDLImportRun *> *PicaExtractPlaceholders(NSArray<RDLImportRun *> *runs,
  ...
  NSArray<NSValue *> *ranges = PicaPlaceholderRanges(joined, &names);
  if ([ranges count] == 0)
    return runs;
...
// on it loses nothing.
static NSArray<RDLImportRun *> *PicaSplitTabs(NSArray<RDLImportRun *> *runs) {
  BOOL any = NO;
  for (RDLImportRun *run in runs)
...
static CGFloat PicaEMUToInches(NSString *emu) {
...
static NSXMLElement *PicaFindDescendant(NSXMLElement *el, NSString *localName) {
  for (NSXMLNode *n in [el children]) {
    if (n.kind != NSXMLElementKind)
      continue;
    NSXMLElement *kid = (NSXMLElement *)n;
    NSString *name = PicaLocal(kid);
    if ([name isEqualToString:@"Fallback"])
      continue;
    if ([name isEqualToString:localName])
      return kid;
    NSXMLElement *found = PicaFindDescendant(kid, localName);
    if (found)
      return found;
  }
  return nil;
...
static void PicaCollectDrawings(NSXMLElement *el, NSMutableArray<NSXMLElement *> *into) {
  for (NSXMLNode *n in [el children]) {
    if (n.kind != NSXMLElementKind)
      continue;
    NSXMLElement *kid = (NSXMLElement *)n;
    ...
    PicaCollectDrawings(kid, into);
  }
}

static NSString *PicaMIMEForPath(NSString *path) {
...
static NSString *PicaRelationshipTarget(RDLZipArchive *zip, NSString *identifier,
                                        NSString *part);
...
static RDLImportBlock *PicaDrawingBlock(NSXMLElement *drawing, PicaDocxContext *ctx) {
  NSXMLElement *extent = PicaFindDescendant(drawing, @"extent");
  CGFloat width = PicaEMUToInches(PicaAttr(extent, @"cx"));
  CGFloat height = PicaEMUToInches(PicaAttr(extent, @"cy"));
  NSXMLElement *blip = PicaFindDescendant(drawing, @"blip");
  NSString *identifier = PicaAttr(blip, @"embed");
  if ([identifier length]) {
    NSString *target = PicaRelationshipTarget(ctx.zip, identifier, ctx.relationshipsPart);
    NSData *bytes = target ? [ctx.zip dataForEntryNamed:[@"word/" stringByAppendingString:target]]
                           : nil;
    NSString *mime = PicaMIMEForPath(target);
  ...
  NSString *name = PicaAttr(PicaFindDescendant(drawing, @"docPr"), @"name");
...
static NSArray<RDLImportBlock *> *PicaDrawingBlocksOfParagraph(NSXMLElement *p,
                                                               PicaDocxContext *ctx) {
  NSMutableArray<NSXMLElement *> *drawings = [NSMutableArray array];
  PicaCollectDrawings(p, drawings);
  if ([drawings count] == 0)
    return nil;
  NSMutableArray<RDLImportBlock *> *blocks = [NSMutableArray array];
  for (NSXMLElement *drawing in drawings) {
    RDLImportBlock *block = PicaDrawingBlock(drawing, ctx);
    if (block)
...
static NSArray<RDLImportRun *> *PicaParagraphRuns(NSXMLElement *p,
                                                  NSMutableArray<NSString *> *collected,
                                                  PicaDocxContext *ctx) {
  NSString *paragraphStyle = PicaVal(PicaKid(PicaKid(p, @"pPr"), @"pStyle"));
  ...
  for (NSXMLNode *n in [p children]) {
    ...
    NSString *name = PicaLocal(el);

    // The short form: instruction and display text in one element.
    if ([name isEqualToString:@"fldSimple"]) {
      NSString *field = PicaMergeFieldName(PicaAttr(el, @"instr"));
      RDLImportRun *run = [[RDLImportRun alloc] init];
      run.fieldName = field;
      run.text = @"";
      for (NSXMLElement *r in PicaKids(el, @"r")) {
        run.text = [run.text stringByAppendingString:PicaRunText(r)];
        if (run.style == nil)
          run.style = PicaRunStyle(PicaKid(r, @"rPr"), ctx.sheet, paragraphStyle);
      }
      if (field) {
        [runs addObject:run];
    ...
    NSXMLElement *fldChar = PicaKid(el, @"fldChar");
    if (fldChar) {
      NSString *type = PicaAttr(fldChar, @"fldCharType");
      if ([type isEqualToString:@"begin"]) {
        instruction = [NSMutableString string];
        inFieldResult = NO;
      } else if ([type isEqualToString:@"separate"]) {
        pendingField = PicaMergeFieldName(instruction);
        instruction = nil;
        inFieldResult = YES;
    ...
    NSXMLElement *instrText = PicaKid(el, @"instrText");
    if (instrText != nil) {
      // The instruction can be spread over several runs.
      if (instruction == nil)
        instruction = [NSMutableString string];
      [instruction appendString:PicaElementText(instrText)];
      continue;
    }
    if (inFieldResult)
    ...
    RDLImportRun *run = [[RDLImportRun alloc] init];
    run.text = PicaRunText(el);
    run.style = PicaRunStyle(PicaKid(el, @"rPr"), ctx.sheet, paragraphStyle);
    [runs addObject:run];
  }
  return PicaSplitTabs(PicaCoalesce(PicaExtractPlaceholders(PicaCoalesce(runs), collected)));
}
...
static RDLImportBlock *PicaParagraphBlock(NSXMLElement *p,
                                         NSMutableArray<NSString *> *collected,
                                         PicaDocxContext *ctx) {
  RDLImportBlock *block = [[RDLImportBlock alloc] init];
  block.kind = RDLImportBlockParagraph;
  block.runs = PicaParagraphRuns(p, collected, ctx);
  NSXMLElement *pPr = PicaKid(p, @"pPr");
  block.styleName = PicaVal(PicaKid(pPr, @"pStyle"));
  if (ctx.sheet) {
    PicaApplyParagraphProperties(block, ctx.sheet.defaultParagraphProperties);
    NSString *styleId = [block.styleName length] ? block.styleName
                                                 : ctx.sheet.defaultParagraphStyleId;
    for (NSXMLElement *properties in PicaStyleChain(ctx.sheet, styleId, @"pPr"))
      PicaApplyParagraphProperties(block, properties);
  }
  PicaApplyParagraphProperties(block, pPr);
  return block;
}

#pragma mark - Tables

static RDLImportBlock *PicaTableBlock(NSXMLElement *tbl,
                                     NSMutableArray<NSString *> *collected,
                                     PicaDocxContext *ctx) {
  RDLImportBlock *block = [[RDLImportBlock alloc] init];
  ...
  for (NSXMLElement *col in PicaKids(PicaKid(tbl, @"tblGrid"), @"gridCol"))
    [widths addObject:@(PicaTwipsToInches(PicaAttr(col, @"w")))];
  block.columnWidths = widths;

  NSMutableArray *rows = [NSMutableArray array];
  for (NSXMLElement *tr in PicaKids(tbl, @"tr")) {
    RDLImportRow *row = [[RDLImportRow alloc] init];
    NSXMLElement *trPr = PicaKid(tr, @"trPr");
    // "Repeat as header row at the top of each page", which like every other
    // toggle can be switched back off explicitly.
    BOOL header = NO;
    row.isHeader = PicaToggleState(trPr, @"tblHeader", &header) && header;
    NSMutableArray *cells = [NSMutableArray array];
    for (NSXMLElement *tc in PicaKids(tr, @"tc")) {
      RDLImportCell *cell = [[RDLImportCell alloc] init];
      NSString *span = PicaVal(PicaKid(PicaKid(tc, @"tcPr"), @"gridSpan"));
      cell.columnSpan = span ? MAX([span integerValue], 1) : 1;
      NSMutableArray *runs = [NSMutableArray array];
      // A cell holds paragraphs; join them, since a report cell is one value.
      for (NSXMLElement *p in PicaKids(tc, @"p")) {
        NSArray *paraRuns = PicaParagraphRuns(p, collected, ctx);
...
static RDLImportSection *PicaSection(NSXMLElement *sectPr) {
  RDLImportSection *section = [[RDLImportSection alloc] init];
  if (sectPr == nil)
    return section;
  NSXMLElement *size = PicaKid(sectPr, @"pgSz");
  if (size) {
    CGFloat w = PicaTwipsToInches(PicaAttr(size, @"w"));
    CGFloat h = PicaTwipsToInches(PicaAttr(size, @"h"));
    if (w > 0)
      section.pageWidth = w;
    if (h > 0)
      section.pageHeight = h;
  }
  NSXMLElement *margins = PicaKid(sectPr, @"pgMar");
  if (margins) {
    section.marginLeft = PicaTwipsToInches(PicaAttr(margins, @"left"));
    section.marginRight = PicaTwipsToInches(PicaAttr(margins, @"right"));
    section.marginTop = PicaTwipsToInches(PicaAttr(margins, @"top"));
    section.marginBottom = PicaTwipsToInches(PicaAttr(margins, @"bottom"));
  }
  NSXMLElement *cols = PicaKid(sectPr, @"cols");
  if (cols) {
    NSString *count = PicaAttr(cols, @"num");
    section.columnCount = count ? MAX([count integerValue], 1) : 1;
    section.columnSpacing = PicaTwipsToInches(PicaAttr(cols, @"space"));
  }
  return section;
}
...
+ (NSArray<NSValue *> *)placeholderRangesIn:(NSString *)text
  ...
  return PicaPlaceholderRanges(text, outNames);
}

// The blocks of one part -- the body, or a header or footer.
static NSArray<RDLImportBlock *> *PicaBlocksOfBody(NSXMLElement *body,
                                                   NSMutableArray<NSString *> *collected,
  ...
  for (NSXMLNode *n in [body children]) {
    ...
    if ([name isEqualToString:@"p"]) {
      ...
      for (RDLImportBlock *drawing in PicaDrawingBlocksOfParagraph(el, ctx)) {
        drawing.sectionIndex = sectionIndex;
        [blocks addObject:drawing];
      }
      RDLImportBlock *block = PicaParagraphBlock(el, collected, ctx);
      ...
      NSXMLElement *sectPr = PicaKid(PicaKid(el, @"pPr"), @"sectPr");
      if (sectPr && sections) {
        while ((NSInteger)[sections count] <= sectionIndex)
          [sections addObject:[[RDLImportSection alloc] init]];
        sections[(NSUInteger)sectionIndex] = PicaSection(sectPr);
        sectionIndex++;
      }
    } else if ([name isEqualToString:@"tbl"]) {
      RDLImportBlock *block = PicaTableBlock(el, collected, ctx);
      block.sectionIndex = sectionIndex;
      [blocks addObject:block];
      ...
      while ((NSInteger)[sections count] <= sectionIndex)
        [sections addObject:[[RDLImportSection alloc] init]];
      sections[(NSUInteger)sectionIndex] = PicaSection(el);
    }
  }
...
static NSXMLElement *PicaPartBody(RDLZipArchive *zip, NSString *part, NSString *rootName) {
  ...
  if (root == nil)
    return nil;
  return rootName ? PicaKid(root, rootName) : root;
}
...
static NSString *PicaRelationshipTarget(RDLZipArchive *zip, NSString *identifier,
  ...
  for (NSXMLNode *n in [[doc rootElement] children]) {
    ...
    if ([PicaAttr(rel, @"Id") isEqualToString:identifier])
      return PicaAttr(rel, @"Target");
  }
  return nil;
}

static NSString *PicaReferenceId(NSXMLElement *sectPr, NSString *kind) {
  for (NSXMLElement *ref in PicaKids(sectPr, kind)) {
    NSString *type = PicaAttr(ref, @"type");
    // "default" applies to every page; first/even are refinements we ignore.
    if (type == nil || [type isEqualToString:@"default"])
      return PicaAttr(ref, @"id");
  }
  return nil;
}
...
+ (RDLImportDocument *)documentFromData:(NSData *)data error:(NSError **)error {
  ...
  for (NSXMLNode *n in [body children]) {
    if (n.kind != NSXMLElementKind)
      continue;
    NSXMLElement *el = (NSXMLElement *)n;
    NSXMLElement *sectPr = [PicaLocal(el) isEqualToString:@"sectPr"]
                               ? el
                               : PicaKid(PicaKid(el, @"pPr"), @"sectPr");
    if (sectPr == nil)
      continue;
    if (out.headerBlocks == nil) {
      ...
      if (hdr) {
        ...
        out.headerBlocks = PicaBlocksOfBody(hdr, collected, nil, ctx);
    ...
    if (out.footerBlocks == nil) {
      ...
      if (ftr) {
        ctx.relationshipsPart =
            [NSString stringWithFormat:@"word/_rels/%@.rels", [target lastPathComponent]];
        out.footerBlocks = PicaBlocksOfBody(ftr, collected, nil, ctx);
        ctx.relationshipsPart = nil;
      }
    }
  }
