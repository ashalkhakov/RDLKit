...
//
// Only what RDL can actually store is offered. PicaRichTextCodec round-trips
...
typedef NS_ENUM(NSInteger, PicaRichTextTrait) {
  PicaRichTextTraitBold,
  PicaRichTextTraitItalic,
  PicaRichTextTraitUnderline,
  PicaRichTextTraitStrikethrough,
};
...
typedef NS_ENUM(NSInteger, PicaTriState) {
  PicaTriStateOff = 0,
  PicaTriStateOn,
  PicaTriStateMixed,
};
...
@interface PicaRichTextState : NSObject
@property (nonatomic, assign) PicaTriState bold, italic, underline, strikethrough;
...
@interface PicaRichTextFormatter : NSObject
