// PicaPageGeometry — where things are on the page, in view points.
//
...
extern const CGFloat PicaPointsPerInch;

// Drag handle kinds, as returned by -itemAtPoint:.
extern NSString * const PicaHandleMove;
extern NSString * const PicaHandleSouthEast;
extern NSString * const PicaHandleEast;
extern NSString * const PicaHandleSouth;

// Tablix preview parts, as returned by +tablix:itemRect:point:column:part:.
extern NSString * const PicaTablixPartHeader;
extern NSString * const PicaTablixPartValue;

// One band's placement, paired with its key so callers never have to index two
// parallel arrays (a previous source of drift).
@interface PicaBandFrame : NSObject
...
@interface PicaPageGeometry : NSObject
...
@property (nonatomic, readonly, copy) NSArray<PicaBandFrame *> *bandFrames;
