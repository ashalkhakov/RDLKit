...
static NSString *PicaHexFromColor(NSColor *color) {
...
static RDLStyle *PicaSparseRunStyle(NSDictionary *attrs, RDLStyle *base) {
...
@end
@implementation PicaRichTextResult
@end
...
+ (PicaRichTextResult *)resultForAttributedString:(NSAttributedString *)text
  ...
  PicaRichTextResult *r = [[PicaRichTextResult alloc] init];
  r.text = flat;
...
+ (BOOL)convert:(NSAttributedString *)text
  ...
  while (paraStart <= len) {
    ...
    while (loc < NSMaxRange(paraRange)) {
      ...
      RDLStyle *sparse = PicaSparseRunStyle(attrs, base);
      if (!PicaStyleIsEmpty(sparse)) {
        run.style = sparse;
        rich = YES;
      }
