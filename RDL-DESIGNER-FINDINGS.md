...
| DAT-07, DAT-09 | Preview renders without asking for parameter values | **fixed, then rebuilt as a panel** — the first fix put the prompts in a bar above the pages, and a real report asks for seven or eight, which is more than a bar can hold without becoming the window. The bar now says what this render is using and opens **Parameters and Data…**: every parameter, scrolling however many there are, and beside them the document each data source reads — in this kit that is the reader's to give too, since a report naming `invoices.json` is asking for a file on this machine. OK renders again, Cancel puts back every value it found. Was **fixed** — the values were asked for, but in a pane of the designer window, which is not where a report is rendered. The preview has the bar a report server puts above the pages: the prompts, on the values the render is using, and View Report to render again with what has been given. The prompts are one view now, shown in both places, so the pane and the bar cannot disagree |
...
| PRB-02 | Clicking a problem starts editing the row | **fixed** — the three columns of the problems list were editable, and nothing anywhere takes an edit from them. On Cocoa a first click only selects, so it never showed; on GNUstep it puts the row into a text field. A check now scans every XIB: a column may be editable only where the pane it belongs to takes the edit. It found the embedded images' type and size columns as well |
| SRC-01 | The source pane is black text on a black ground | **fixed** — the text view said nothing about its colours, so it took the platform's, and GNUstep's are not Cocoa's. It names both now, as the Code editor's already did, and a check reads them back and refuses a pane whose ink and paper are the same colour |
...
| — | A tablix cannot be resized as a whole | **fixed** — a region is as wide as its columns and as tall as its rows, and resizing it set its own width and height, which left the grid inside it the size it was: the region drew at one size and its lines at another. A resize shares the change out among the body's columns and rows now, each keeping its share, with what the row headers and column headings take left alone. One undo step |
| — | A drag handle under another item selects that item instead | open |
| INS-04 | The drag shows "cannot drop" while dropping works | **stated explicitly; not reproducible here** — on Cocoa a table's drags already allow a copy inside the application and nothing outside it, so nothing here shows the badge. Every table that drags now says so outright rather than relying on the platform's default, which is what a wrong badge would come from. Worth re-checking on Linux |
| CAN-04 | Holding an arrow key stutters | **fixed** — on GNUstep nothing moved until the key was let go, and then the whole burst arrived at once. Every repeat published a change, and every pane that listens read the report again — the inspector filling fifty controls among them — which is slower than the key repeats. A burst now tells them once, when the key is let go; the canvas redraws itself on every press, since it is the one that has to keep up. It stays one undo step |
