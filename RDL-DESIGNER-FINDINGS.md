...
| CAN-04 | Holding an arrow key stutters | **open (GS), and needs the platform** — each repeat moves the item and redraws the page, which AppKit coalesces into one draw a turn; nothing here is slow enough to see. Guessing at it from this side would be shipping a change nobody can measure: it wants a profile on Linux to say what the repeat is spending its time in |
