// RDLInPlaceEditor — the double-click-to-edit session on the canvas.
//
// Split out of RDLCanvasView, which had eight ivars and a dozen methods
...
@class RDLInPlaceEditor;
@class RDLItem;
@class RDLPageGeometry;

@protocol RDLInPlaceEditorHost <NSObject>
...
@interface RDLInPlaceEditor : NSObject
- (instancetype)initWithContext:(RDLEditingContext *)context hostView:(NSView *)hostView;
@property (nonatomic, weak) id<RDLInPlaceEditorHost> host;
