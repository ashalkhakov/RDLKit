...
typedef NS_ENUM(NSInteger, RDLNewReportSource) {
  RDLNewReportSourceUnspecified = 0,
  RDLNewReportSourceBlank,
  RDLNewReportSourceWordDocument,
};

// The result of making a report, with everything the panel needs to say how it
// went. `report` is nil exactly when `error` is set.
@interface RDLNewReportOutcome : NSObject
@property (nonatomic, readonly, strong) RDLReport *report;
@property (nonatomic, readonly, assign) RDLNewReportSource source;
...
@interface RDLNewReport : NSObject
